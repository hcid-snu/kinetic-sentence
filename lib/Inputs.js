var Inputs= function() {
  ...
  this.types = {Negative:['fadeIn', 'scaleDown', 'raining', 'angry'], Positive:['fadeIn', 'scaleUp', 'happy', 'wigglyScale']}
  this.neuTypes = ['fadeIn']//['dropIn', 'fadeIn', 'fadeOut', 'spinIn']
  this.negTypes = ['fadeIn', 'scaleDown', 'raining', 'angry']
  this.posTypes = ['fadeIn', 'scaleUp', 'happy', 'wigglyScale']
  ...
  this.generate = function() {
    ...
    d3.layout.cloud().size([this.width, this.height])
      .words(self.words.map(function(d,i) {
        return {text: d, energy:self.getEnergy(i) ,size: 16 + self.getEnergy(i) * 10, type:self.getType(i)}
      }))
      .padding(0)
  ...
  this.setMotion = (function() {
    return function (self,d, delays) {
      if (d.type === 'dropIn') {
        ...
        var duration = getDuration(800, 60, d.energy)
        var delayLetter = 0; 
        ...
        var duration = getDuration(60, 6, d.energy);
        var delayLetter = getDuration(800, 100, d.energy);
        d3.select(self).raining(20, duration, delays, delayLetter); 
        return delays += duration * d.letters.length + delayLetter * d.letters.length;
      } else if (d.type === 'scaleUp') {
        var duration = getDuration(600, 100, d.energy);
        ...
        var duration = getDuration(80, 10, d.energy);
        d3.select(self).angry(getDuration(1.25, -.05, d.energy), getDuration(50, -8, d.energy), 4, duration, delays);
        return delays += duration * 14;
      } else if (d.type === 'wigglyScale') {
        var duration = getDuration(200, 20, d.energy);
        d3.select(self).wigglyScale(duration, delays); 
        return delays += (duration * d.letters.length*2.5) ;
      } else if (d.type === 'happy') {
        var duration = getDuration(150, 10, d.energy);
        d3.select(self).happy(getDuration(30, -4, d.energy), getDuration(-60, 8, d.energy), duration, delays, 200); 
