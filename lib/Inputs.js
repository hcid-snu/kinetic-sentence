var Inputs= function() {
  ...
  this.negTypes = ['fadeIn', 'scaleDown', 'raining', 'jitter2']
  ...
  this.fontSize = 36; this.fontUnit = 8;
  this.totalDelays = 0;
  ...
  this.generate = function() {
    ...
    self.fontSize = getDuration(36, -3, self.avgEnergy);
    self.fontUnit = getDuration(8, -1, self.avgEnergy);
    //console.log("width: " + self.width + " / font: " + self.fontSize + " / fontUnit: " + self.fontUnit);
  ...
  this.setMotion = (function() {
    return function (self,d, delays) {
      if (d.type === 'dropIn') {
        ...
        d3.select(self).raining(d.size*0.6, duration, delays, delayLetter); 
  ...
  this.draw = function (words) {
    ...
    d3.select("#canvas").append("svg")
      ...
      .append("g")
        .attr("transform", "translate("+ [self.width/2, self.height/2 + 50] + ")")
      .selectAll("g")
        .data(words)
      .enter().append("g")
      ...
      for (var j = i+1; j < this.words.length ; j++) {
          ...
          for (var n = 1; n < this.posTypes.length; n ++) {
            for (var x = 0; x < energy_arr.length; x++) {
              for (var y = 0; y < energy_arr.length; y++) {
                exp_types.push([i, j, m-1, n-1, x, y])
              }
            }
          }
        }
        for (var m = 1; m < this.negTypes.length ; m ++) {
          for (var n = 1; n < this.negTypes.length; n ++) {
            for (var x = 0; x < energy_arr.length; x++) {
              for (var y = 0; y < energy_arr.length; y++) {
                exp_types.push([i, j, m + 3 -1, n +3 -1, x, y])
              }
            }
          }
        }
