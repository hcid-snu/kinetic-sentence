...
(function() {
    ...
    = function(duration, delayWord) {
        ...
        .each(function(l, li) {
            var self = this;
            wigglyScaleChain(self, d, 2);//6 + Math.random() * 2);
        });
    ...
    d3.selection.prototype.angry 
    = d3.selection.enter.prototype.angry
    = function(scale, x1, y1, duration, delayWord) {
        var self = this;
        var i = self.attr("class").split(" ")[1]
        var d = self.datum();
        var setPos = setPosition();
    ...
    function angryChain(self, d, repCount) {
    ...
    d3.selection.prototype.happy 
    = d3.selection.enter.prototype.happy
    = function(deg, y1, duration, delayWord, delayLetter) {
        ...
        .each(function(l, li) {
            var self = this;
            happyChain(self, d, 1, duration, delayLetter*d.letters.length + delayLetter*(d.letters.length-li -1));
        });

        return self;
    }

    function happyChain(self, d, repCount, duration, newDelay) {
        //repCount = repCount | 6;
        var letter = d3.select(self); // select a letter;
        (function repeat() {
            if (repCount-- >= 1) {
    ...
    = function(y1, duration, delayWord, delayLetter) {
        ...
        .delay(function(l, li) {
            return delayWord + delayLetter +  delayLetter * Math.random() ;
        })
        .each(function(l, li) {
            var self = this;
            rainingChain(self, d, 10);
        });
    ...
    function rainingChain(self, d, repCount) {
        ...
        (function repeat() {
            if (repCount-- >= 1) {
                letter = letter.transition()
                .style("opacity", repCount/initCount )
                .attr("transform", function(l, li) {
                    var x = y = 0;
                    var scale = 1.0;
                    if (l.move !== undefined) {
                ...
                .text(function(){ return String.fromCharCode(Math.floor(Math.random()*128))})
                .transition()
    ...
    = function(duration, delayWord, delayLetter) {
        ...
        .attr("transform", function(l, li) {
            setPos(d,i,  l, li);
            return getPosition(d, l, 0, 0, 1.0, 0);
        })
        .transition()
        ...
        .duration(duration)
        .delay(function(l, li) {
            return delayWord + delayLetter * (li +1);
        })
